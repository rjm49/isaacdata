...
class hwgengen2:
    def __init__(self, assts, batch_size=512, pid_override=None, FRESSSH=False, qid_override=all_qids):
    ...
    def __iter__(self):
        ...
        for i, ts, gb_id, gr_id in zip(self.assid_list, self.ts_master_list, self.gb_id_list, self.gr_id_list):
            ...
            # print("#{}: asst {}: gbd {} to grp {} at {}".format(ix, i, gb_id, gr_id, ts))
            ...
            for psi in students:  # set up the training arrays here
                ...
                if tripat is None:
                    ...
                    for hx in hexagons:
                        ...
                        # print("DIM", s_psi.shape, x_psi.shape, u_psi.shape)

                        S.append(s_psi)
                        # print("X with {} XP".format(numpy.sum(x_psi)))
                        X.append(x_psi)
                        U.append(u_psi)
                        # X.append(numpy.zeros(1))
                        # U.append(numpy.zeros(1))
                        y.append([hx])
                        assids.append(i)
